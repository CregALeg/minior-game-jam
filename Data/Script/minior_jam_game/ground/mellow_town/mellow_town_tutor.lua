...
function mellow_town_tutor.Tutor_Can_Remember(member)
  return GAME:CanRelearn(member)
end

function mellow_town_tutor.Tutor_Can_Forget(member)
  return GAME:CanForget(member)
end

function mellow_town_tutor.Tutor_Can_Tutor(member, tutor_moves)
...
function mellow_town_tutor.Tutor_Remember_Flow(price)
  ...
  local move = ""
	...
	while state > -1 do
		if state == 0 then
			...
			local chosen_member = AssemblySelectMenu.run(mellow_town_tutor.Tutor_Can_Remember, false)
      ...
      if result ~= "" then
        ...
        local moveEntry = _DATA:GetSkill(move)
			...
			if learnedMove then
				...
				mellow_town_tutor.Tutor_Sequence()
...
function mellow_town_tutor.Tutor_Forget_Flow()
  local state = 0
  local member = nil
  local move = ""
  ...
  while state > -1 do
    ...
    if state == 0 then
      ...
      local chosen_member = AssemblySelectMenu.run(mellow_town_tutor.Tutor_Can_Forget, false)
...
function mellow_town_tutor.Tutor_Teach_Flow(tutor_moves)
	...
	while state > -1 do
		if state == 0 then
			...
			local chosen_member = AssemblySelectMenu.run(function(chara) return mellow_town_tutor.Tutor_Can_Tutor(chara, tutor_moves) end, false)
...
function mellow_town_tutor.MOVES_COUNTER_Action(obj, activator)
  ...
  -- end
	...
	while state > -1 do
		if state == 0 then
			...
			repeated = true
			if result == 1 then
			    mellow_town_tutor.Tutor_Remember_Flow(price)
			elseif result == 2 then
				mellow_town_tutor.Tutor_Forget_Flow()
