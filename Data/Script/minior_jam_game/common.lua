...
function COMMON.EndDayCycle()
  ...
  -- Wares Mart Gen
  --2-3 orbs, always
  type_count = math.random(2, 3)
	for ii = 1, type_count, 1 do
		local base_data = COMMON.ORBS[math.random(1, #COMMON.ORBS)]
		table.insert(SV.wares_shop, base_data)
	end
  --1 to 2 wands
  type_count = math.random(2, 3)
	for ii = 1, type_count, 1 do
		local base_data = COMMON.WANDS[math.random(1, #COMMON.WANDS)]
		table.insert(SV.wares_shop, base_data)
	end
  -- 0 to 2 TMS
  type_count = math.random(0, 2)
	for ii = 1, type_count, 1 do
		local base_data = COMMON.TM[math.random(1, #COMMON.TM)]
		table.insert(SV.wares_shop, base_data)
	end
  --1 special item, always
  type_count = 1
